...
export class Func037 implements IFuncOrigin {
	...
	operator: IFuncOperatorOrigin[] = [{
		...
		oper: [
			...
			[center, 1280, 720, 992, 600, 1154, 650, 1000],		//	点击 创建队伍
      ...
      return true;
		...
		if (thisScript.oper({
			...
			if (thisScript.global.team_up_lagTime) {
				...
				if (new Date().getTime() - thisScript.global.team_up_lagTime.getTime() > time * 60000) {
			...
			let target = thisconf && thisconf.target || '金币妖怪';
			let result = thisScript.findText(target as string, 5000, thisOperator[0].oper[2], '包含');
