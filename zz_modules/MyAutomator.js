...
function MyAutomator(tapType) {
    ...
    if (tapType == 1) {
        var BezierSwipe = require('./bezierSwiper');
        this.RA = new RootAutomator();
        this.bezierSwiper = new BezierSwipe(this.RA);
    } else if (tapType == 2) {
        var that = this;
...
MyAutomator.prototype = {
    ...
    swipe: function (x0, y0, x1, y1, delay) {
        if (this.tapType == 0) {
            this.bezierSwiper(x0, y0, x1, y1, delay);
        } else if (this.tapType == 1) {
            this.bezierSwiper(dm.heightPixels - y0, x0, dm.heightPixels - y1, x1, delay);
        } else if (this.tapType == 2) {
